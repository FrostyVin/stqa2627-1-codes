...
In this exercise, we will implement test isolation using `unittest.mock.patch` and mock response objects. Our main goal is to test the `IMDb` client without consuming third-party API quotas or relying on external network calls, simulating both successful responses and error states under controlled test conditions.

First of all, examine the provided `models/imdb.py` file to review its API methods and inspect `tests/fixtures/imdb_responses.json` to familiarize yourself with the recorded fixture responses.
